...
function AlienLaunchMarker:update(dt)
    ...
    -- perform everything here as long as we have launched
    else

      if not self.alien.hitted and not self.splitted then
        if love.keyboard.wasPressed('space') then
          ...
          for i = 1, 2 do
            ...
            if i == 1 then
              alien = Alien(self.world, 'round', xPos, yPos + 40, 'Player')
            else
              alien = Alien(self.world, 'round', xPos, yPos - 40, 'Player')
            end
            ...
            -- apply the difference between current X,Y and base X,Y as launch vector impulse
            if i == 1 then
              alien.body:setLinearVelocity(xVel, yVel + 10)
            else
              alien.body:setLinearVelocity(xVel, yVel - 10)
            end
            -- make the alien pretty bouncy
